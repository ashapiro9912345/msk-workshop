...
try {
    ...
    if (-not $bastionId -or $bastionId -eq "") {
        Write-Host "[X] Bastion host not found. Set create_bastion=true in terraform.tfvars and run terraform apply" -ForegroundColor Red
        exit 1
    }

    Write-Host "[OK] Bastion Instance ID: $bastionId" -ForegroundColor Green
    Write-Host "[OK] Aurora Endpoint: $auroraEndpoint" -ForegroundColor Green
    Write-Host ""
...
try {
    ...
    Write-Host "[OK] Session Manager plugin is installed" -ForegroundColor Green
} catch {
    Write-Host "[X] Session Manager plugin not found" -ForegroundColor Red
    Write-Host ""
