...
try {
    ...
    Write-Host "✓ Service Role: $roleArn" -ForegroundColor Green
} catch {
    Write-Error @"
...
try {
    ...
    if ($debeziumPlugin) {
        Write-Host "✓ Debezium Plugin: $($debeziumPlugin.name) (revision $($debeziumPlugin.latestRevision.revision))" -ForegroundColor Green
    } else {
        Write-Warning "Debezium plugin not found. Ensure Terraform created it with create_connectors=true"
    }

    if ($jdbcPlugin) {
        Write-Host "✓ JDBC Plugin: $($jdbcPlugin.name) (revision $($jdbcPlugin.latestRevision.revision))" -ForegroundColor Green
    } else {
...
try {
    ...
    Write-Host "✓ Bootstrap Brokers: $bootstrapBrokers" -ForegroundColor Green
} catch {
...
foreach ($connector in $deploymentResults.Keys) {
    $status = if ($deploymentResults[$connector]) { "✓ SUCCESS" } else { "✗ FAILED/SKIPPED" }
